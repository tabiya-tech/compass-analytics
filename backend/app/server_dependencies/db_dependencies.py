...
from app.roles.repository import ROLES_COLLECTION, USER_ROLES_COLLECTION
...
class AnalyticsDBProvider:
    ...
    async def initialize_mongo_db(db: AsyncIOMotorDatabase) -> None:
        ...
        await db[ROLES_COLLECTION].create_index("name", unique=True, name="roles_name_unique")
        await db[USER_ROLES_COLLECTION].create_index(
            [("user_id", 1), ("role_id", 1), ("institution_id", 1)],
            unique=True,
            name="user_roles_unique_tuple",
        )
