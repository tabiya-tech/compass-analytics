...
def main():
    ...
    # Optional extra custom domain aliases for the frontend (JSON array).
    ...
    dns_stack_ref = pulumi.StackReference(f"tabiya-tech/analytics-dns/{stack_name}")
    dns_zone_name = getstackref(dns_stack_ref, "dns_zone_name")

    # Get stack reference for frontend
    frontend_stack_ref = pulumi.StackReference(f"tabiya-tech/analytics-frontend/{stack_name}")
    ...
    backend_stack_ref = pulumi.StackReference(f"tabiya-tech/analytics-backend/{stack_name}")
