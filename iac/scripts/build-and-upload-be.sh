...
function build_and_upload_be_docker_img() {
  ...
  _artifact_name="$_region-docker.pkg.dev/$_project_id/docker-repository/analytics-backend:$_artifact_version"
...
echo "info: building and uploading analytics-backend:$docker_artifact_version artifacts to $region/$project_id from $source_path"

# Ensure the version.json directory exists.
mkdir -p "$(dirname "$version_json_filename")"

# Create version.json if it doesn't exist (the analytics backend does not commit one;
# it reads version info from environment variables at runtime, so the file is generated here for the report).
