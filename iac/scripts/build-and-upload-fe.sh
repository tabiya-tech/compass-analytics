...
function check_args() {
  ...
  This script performs the following steps:
    ...
    4. Optionally uploads sourcemaps to Sentry if SENTRY_AUTH_TOKEN is set and a sentry:sourcemaps script exists.
...
source_path="$ROOT_PATH/frontend"
...
# The analytics frontend is a Vite app; the build output directory is 'dist'.
version_json_filename="$source_path/dist/data/version.json"
...
# Upload sourcemaps to Sentry only if a sentry:sourcemaps script exists and a token is set.
if [ -n "$SENTRY_AUTH_TOKEN" ]; then
  if grep -q '"sentry:sourcemaps"' "$source_path/package.json"; then
    echo "info: uploading sourcemaps to sentry"
    yarn --cwd "$source_path" run sentry:sourcemaps || exit 1
  else
    echo "warning: no sentry:sourcemaps script found in package.json, skipping uploading sourcemaps to sentry"
  fi
else
  echo "warning: SENTRY_AUTH_TOKEN is not set, skipping uploading sourcemaps to sentry"
fi

# Ensure the version.json directory exists (the frontend may not ship a data/version.json by default)
...
tar -czf "./$frontend_build_artifact_filename" -C "$source_path"/dist . || exit 1
