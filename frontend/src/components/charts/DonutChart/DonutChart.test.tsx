...
describe("DonutChart", () => {
  it("should name the plot and draw a segment per slice", () => {
    ...
    expect(screen.getAllByTestId(DATA_TEST_ID.SEGMENT)).toHaveLength(GROUPS.length);
  });
  ...
  it("should draw a single slice as a full ring, with no gap cut into it", () => {
    // GIVEN a whole made of one slice
    // WHEN it is rendered
    render(<DonutChart label={LABEL} slices={SINGLE_SLICE} />);

    // THEN it is drawn as two half-arcs closing a full ring, since a gap here
    // would read as a missing segment rather than as 100%
    const [actualSegment] = screen.getAllByTestId(DATA_TEST_ID.SEGMENT);
    expect(actualSegment.getAttribute("d")?.match(/A/g)).toHaveLength(4);
  });
